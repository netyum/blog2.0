 ...
 *
 * The main property of Menu is {@a items}, which specifies the possible items in the menu.
...
class Menu extends Widget
{
	 ...
	 * <ul>
	 * <li>label: string, optional, specifies the menu item label. When {@a encodeLabel} is true, the label
	 * will be HTML-encoded. If the label is not specified, it defaults to an empty string.</li>
	 * <li>url: string or array, optional, specifies the URL of the menu item. It is passed to {@a Html::normalizeUrl}
	 ...
	 * If a menu item is active and {@a activeClass} is not empty, its CSS class will be appended with {@a activeClass}.
	 * If this option is not set, the menu item will be set active automatically when the current request
	 * is triggered by {@a url}. Note that the GET parameters not specified in the 'url' option will be ignored.</li>
	 * <li>template: string, optional, the template used to render this menu item.
	 * When this option is set, it will override the global setting {@a itemTemplate}.
	 * Please see {@a itemTemplate} for more details. This option has been available since version 1.1.1.</li>
	 * <li>aOptions: array, optional, additional HTML attributes to be rendered for the a or span tag of the menu item.</li>
	 ...
	 * When this option is set, the {@a submenuHtmlOptions} property will be ignored for this particular submenu.
	 ...
	 * the token "{menu}" will be replaced with the corresponding menu a or text.
	 ...
	 * The activated parent menu items will also have its CSS classes appended with {@a activeCssClass}.
	 * Defaults to false.
	 */
	...
	/**
	 * @var string the HTML element name that will be used to wrap the label of all menu as.
	 ...
	 * @since 1.1.4
	 */
	public $aLabelWrapper;
	/**
	 * @var array HTML attributes for the as' wrap element specified in
	 * {@a aLabelWrapper}.
	 * @since 1.1.13
	 */
	public $aLabelWrapperHtmlOptions=array();
	/**
	 ...
	 * This method mainly normalizes the {@a items} property.
	...
	/**
	 * Calls {@a renderMenu} to render the menu.
	 */
	public function run()
	{
	 ...
	 * It may have three other optional elements: 'items', 'aOptions' and 'itemOptions'.
	 */
	 ...
	 * @param array $item the menu item to be rendered. Please see {@a items} on what data might be in the item.
	 * @return string
	 * @since 1.1.6
	 */
	protected function renderMenuItem($item)
	{
		if(isset($item['url']))
		{
			$label=$this->aLabelWrapper===null ? $item['label'] : Html::tag($this->aLabelWrapper, $this->aLabelWrapperHtmlOptions, $item['label']);
			return Html::a($label,$item['url'],isset($item['aOptions']) ? $item['aOptions'] : array());
		}
		else
			return Html::tag('span',isset($item['aOptions']) ? $item['aOptions'] : array(), $item['label']);
	}

	/**
	 * Normalizes the {@a items} property so that the 'active' state is properly identified for every menu item.
